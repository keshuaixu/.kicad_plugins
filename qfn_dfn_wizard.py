...
class QFNWizard(FootprintWizardBase.FootprintWizard):
    ...
    def GetDescription(self):
        return "Quad/Dual Flat No-lead (QFN/DFN) footprint wizard"

    def GenerateParameterList(self):

        self.AddParam("Pads", "n_x", self.uInteger, 25, multiple=1, min_value=0)
        self.AddParam("Pads", "n_y", self.uInteger, 25, multiple=1, min_value=0)
    ...
    def BuildThisFootprint(self):
        ...
        pads_per_row_x = int(self.pads['n_x'])
        pads_per_row_y = int(self.pads['n_y'])

        row_len_x = (pads_per_row_x - 1) * pad_pitch
        row_len_y = (pads_per_row_y - 1) * pad_pitch
        ...
        if self.epad['epad'] == True:
            ...
            epad_num = self.pads['n_x'] * 2 + self.pads['n_y'] * 2 + 1
        ...
        clip_x = row_len_x / 2 + self.pads['pitch']
        clip_y = row_len_y / 2 + self.pads['pitch']

        self.draw.Polyline( [ [ clip_x, -h/2-offset], [ w/2+offset,-h/2-offset], [ w/2+offset, -clip_y] ] ) # top right
        self.draw.Polyline( [ [ clip_x,  h/2+offset], [ w/2+offset, h/2+offset], [ w/2+offset,  clip_y] ] ) # bottom right
        self.draw.Polyline( [ [-clip_x,  h/2+offset], [-w/2-offset, h/2+offset], [-w/2-offset,  clip_y] ] ) # bottom left

        # Add pin-1 indication as per IPC-7351C
        self.draw.Line(-clip_x, -h/2-offset, -w/2-pad_length/2, -h/2-offset)
